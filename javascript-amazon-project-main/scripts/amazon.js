...
loadProducts(renderProductsGrid); // so here we send the request but reponse is no loaded and products array is still empty; to solve this wwe need to wait for HTTPrequest to finish and response to come back; so how do we that?
// solution is to run this after eventListener of xhr; so put below code in the frn and provide it to loadPRoduct i mean export
